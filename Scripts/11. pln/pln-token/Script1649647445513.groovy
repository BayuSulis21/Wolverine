...
//get data excel
TestData input = findTestData('11. pln/pln-token')
for (int baris = 1; baris <= input.getRowNumbers(); baris++) {
	...
	Mobile.comment('Klik Menu Token PLN')
	Mobile.tap(findTestObject('Object Repository/pln-token/Button - Token PLN'), 0)
	Mobile.takeScreenshot()
	Mobile.delay(2)
	
	Mobile.comment('Cari No. Pelanggan')
	Mobile.setText(findTestObject('Object Repository/pln-token/Eno_pelanggan'), input.getValue('no_pelanggan', baris), 0)
	Mobile.takeScreenshot()
	Mobile.delay(2)
	
	Mobile.comment('Pilih amount token')
	String str_amount_token = input.getValue('token', baris)
	Mobile.tap(findTestObject('Object Repository/pln-token/amount-token',[('token'):str_amount_token]), 0)
	Mobile.takeScreenshot()
	Mobile.delay(2)
	
	Mobile.comment('Klik button Lanjutkan')
	Mobile.tap(findTestObject('Object Repository/pln-token/Button - Lanjutkan'), 0)
	Mobile.takeScreenshot()
	Mobile.delay(2)

	Mobile.comment('Klik button bayar')
	Mobile.tap(findTestObject('Object Repository/pln-token/Button - Bayar'), 0)
	Mobile.takeScreenshot()
	Mobile.delay(2)
	...
	if (Mobile.verifyElementVisible(findTestObject('Object Repository/pln-token/View - Transaksi receipt'), 5, FailureHandling.OPTIONAL)) {
		Mobile.tap(findTestObject('Object Repository/pln-token/View - Transaksi receipt'), 0)
		Mobile.takeScreenshot()
		Mobile.delay(2)
	}
	
	Mobile.comment('Kembali ke menu utama')
	Mobile.tap(findTestObject('Object Repository/pln-token/Btn - close'), 0)
	Mobile.delay(2)
