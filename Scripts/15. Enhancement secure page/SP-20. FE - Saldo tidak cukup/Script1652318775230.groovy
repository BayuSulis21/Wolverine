...
//Get Balance
def responseObj_balance = WS.sendRequest(findTestObject('Object Repository/Secure page/GetBalance'))
WS.verifyResponseStatusCode(responseObj_balance, 200)
JsonSlurper slurper_balance = new JsonSlurper()
Map parsedJson_balance = slurper_balance.parseText(responseObj_balance.getResponseText())
String value_amount = parsedJson_balance.get("data").get("balance_amount")
System.out.println(value_amount)
WebUI.comment("Saldo Awal: "+value_amount)
...
//Inquiry 
	...
	def tagihan=value_amount.toInteger()+1000
	String amount=tagihan.toString()
	
	//random trxID
	String trxID="AA1610000"+Math.abs(new Random().nextInt() % 999999)
	System.out.println(trxID)
	String merchantURL="https://ottoapi.pactindo.com/ottocash-admin/ind/back"
	String successURL="https://ottoapi.pactindo.com/ottocash-admin/ind/success"
	String failedURL="https://ottoapi.pactindo.com/ottocash-admin/ind/failed"
	...
	WebUI.comment("Error Message: "+parsedJson.get("fullMessage"))
