...
import org.apache.commons.codec.binary.Base64;
	...
	WS.callTestCase(findTestCase('15. Enhancement secure page/SP-04. BE - success pembayaran tagihan'), null)
	...
	String referenceNumber=GlobalVariable.referenceNumber
	String partnerRefundId=Math.abs(new Random().nextInt() % 9999)
	...
	for (int baris = 1; baris <= 2; baris++) {
		...
		if (baris==1) {
			def amount=GlobalVariable.amount_securepage.toInteger()/2
			String amount_refund=amount.toString()
			String jsonbody_refund = '{"phoneNumber": "'+phoneNumber+'","referenceNumber": "'+referenceNumber+'","partnerRefundId": "'+partnerRefundId+'","amount": '+amount_refund+'}'
			WebUI.comment(jsonbody_refund)
			String jsonbody_replace_refund = '{phoneNumber:'+phoneNumber+',referenceNumber:'+referenceNumber+',partnerRefundId:'+partnerRefundId+',amount:'+amount_refund+'}'
			...
			WebUI.comment("REFUND YANG KE-"+baris+" : "+amount_refund)
			...
			def amount=(GlobalVariable.amount_securepage.toInteger()/2)+5000
			String amount_refund=amount.toString()
			String jsonbody_refund = '{"phoneNumber": "'+phoneNumber+'","referenceNumber": "'+referenceNumber+'","partnerRefundId": "'+partnerRefundId+'","amount": '+amount_refund+'}'
			WebUI.comment(jsonbody_refund)
			String jsonbody_replace_refund = '{phoneNumber:'+phoneNumber+',referenceNumber:'+referenceNumber+',partnerRefundId:'+partnerRefundId+',amount:'+amount_refund+'}'
