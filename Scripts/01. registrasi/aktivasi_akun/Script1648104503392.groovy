...
if (Mobile.verifyElementVisible(findTestObject('aktivasi/menu'), 5, FailureHandling.OPTIONAL)) {
    Mobile.takeScreenshot()

    Mobile.delay(1) 
} else {
    Mobile.comment('Checklist')
    Mobile.tap(findTestObject('Object Repository/aktivasi/checklist'), 0)
    Mobile.delay(1)

    Mobile.comment('Klik Aktifkan')
    Mobile.tap(findTestObject('Object Repository/aktivasi/Aktifkan'), 0)
    Mobile.takeScreenshot()
	...
	if (WS.verifyResponseStatusCode(responseObj, 200))
	{
	    ...
	    Mobile.comment('Klik Selesai Aktivasi')
	    Mobile.tap(findTestObject('aktivasi/BtnSelesai'), 0)
	    Mobile.takeScreenshot()
	    Mobile.delay(2)
